#Codecademy Pokemon project for pythonclass Pokemon:
  def __init__(self, name, level, type1, max_health, current_health, base_attack, dead=False):
    ...
    return print(" Name: {p1} \n Level: {l1} \n Type: {t1} \n Max Health: {m1} \n Current Health: {h1} \n".format(p1=self.name, l1=self.level, t1=self.type1, m1=self.max_health, h1=self.current_health))
  ...
  def lose_health(self,hit):
    ...
    if self.current_health < 1:
      print("{p1} has died. RIP".format(p1=self.name))
    else:
  ...
  def attack(self, rival):
    ...
    rival.dead = dead
    if self.current_health < 1:
      return print("{} is dead and cannot attack anyone".format(self.name))
    if rival.current_health < 1:
      rival.dead = True
      return print(rival.name, " is dead you cannot attack.\n"), rival.dead
...
pika = Pokemon("pikachu", 4, "ice", 99, 45, 10)
meow = Pokemon("meowf", 4, "grass", 99, 34, 13)
jigg = Pokemon("jigglypuff", 4, "fire", 99, 39, 11)
spiff = Pokemon("spiffamungus", 2, "ice", 99, 69, 12)
